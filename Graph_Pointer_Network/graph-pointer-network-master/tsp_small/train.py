...
if __name__ == "__main__":
    ...
    size = int(args['size'])  #number of nodes
    ...
    model = GPN(n_feature=2, n_hidden=128).cpu()
    # load model
    ...
    for epoch in range(n_epoch):
        for i in tqdm(range(steps)):
            ...
            X = np.random.rand(B, size, 2)#生成数量为B的样本，每个样本 包括 数量为 size,属性数量为 2的结点。

            X = torch.Tensor(X).cpu()
            
            mask = torch.zeros(B,size).cpu()
            ...
            if i % 50 == 0:
                ...
                mask = torch.zeros(B_val,size).cpu()
